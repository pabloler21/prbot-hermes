...
hermes_queue/      # arq queue layer (Python):
  ...
  jobs/digest.py     # build_pr_digest: format open PRs as Markdown (Phase 6a, pure/testable)
  jobs/reports.py    # build_stale_pr_report + build_weekly_summary (Phase 6b cron reports)
  jobs/alerts.py     # format_new_issues + format_deploys (Phase 6b poll formatters, pure)
  poll_state.py      # Phase 6b poll state: Redis cursor + capped seen-set (dedup)
  guardrails.py      # repo allowlist (deterministic) + allowed_repos() to iterate
  github_client.py   # comment + labels + list open/merged PRs + closed/new issues (GitHub REST)
  discord_client.py  # post_to_discord via incoming webhook (digest/alert delivery, splits >2000)
  ...
  workers/post_comment_worker.py  # arq worker: tasks (post_comment, apply_issue_labels) + 5 cron/poll jobs (tz UTC-3)
